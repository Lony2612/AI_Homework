...
def seed_fill(img, seeds, pthresh, rthresh):
    ...
    while len(stack_list)!=0:
        if count% 10000 == 0:
            print("expand %d, points %d"%(count, len(stack_list)))
            stack_len = len(stack_list)
            if stack_len > 14000:
                stack_list = stack_list[stack_len-14000:stack_len]
            print("points %d"%(len(stack_list)))
        # 获取需要扩展的点的坐标
...
if __name__ == '__main__':
    ...
    # 定义衣物位移
    shift_list = [[0,0],[-8,0],[-20,0],[14,0]]

    for ii in range(1,n_group):
        # 构造匹配服装的路径
        ...
        if cover_list[ii] == 2:
            changeClothes(img_shirt, img_skirt, labels, HorShift, VerShift, "sh_output%d.jpg"%(ii+1))
        # 如果上衣覆盖下装，则把上衣加到下装图片中
        else:
            changeClothes(img_skirt, img_shirt, labels, HorShift, VerShift, "sh_output%d.jpg"%(ii+1))
