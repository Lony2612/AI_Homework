...
if __name__ == '__main__':
    ...
    # seed_list = [(1110,1499),(1110,1499),(1110,1499),(1110,1499)]
    ...
    for ii in range(0,n_group):
        ...
        # 构造匹配服装的路径
        img1_path = "./%s_input%d.jpg"%(group_name[ii],cov)
        img = cv2.imread(img1_path)
        print(img1_path)
        (B, G, R) = cv2.split(img)
        ...
        if cov == 2:
            changeClothes(img_shirt, img_skirt, labels, HorShift, VerShift, "auto_output%d.jpg"%(ii+1))
        # 如果上衣覆盖下装，则把上衣加到下装图片中
        else:
            changeClothes(img_skirt, img_shirt, labels, HorShift, VerShift, "auto_output%d.jpg"%(ii+1))
